...
<form  class="navbar-form navbar-left" role="search" action="/Film/auth/getFilm?naziv=${naziv}" method="get">

          <input type="text" name="naziv" class="form-control" placeholder="Search">
...
<table class="table table-striped table-hover ">
  <thead>
			<tr class="active">
				...
				<tr class="active">
					<td><img src="/Film/auth/getSlikaFilma?idFilma=${film.idfilm}&ocena=${ocena}&komentar=${komentar}" height="80" width="60"/></td>
					<td>${film.naziv}</td>
					<td>${film.zanr.naziv}</td>
					<td>${film.kategorija.naziv}</td>
					<td>${film.reziser}</td>
					<td>${film.trajanje}</td>
				</tr>
  </tbody>
...
<form action="/Film/filmovi/dodajKomentar?idfilm=${film.idfilm}" method="post">
	<table class="table table-striped table-hover ">
	 <thead>
			<tr class="active">
			<th>Ocena</th>
			<th>Komentar</th>
				...
				<tr class="active">
				<td><input type="text" name="ocena" path="ocena"/></td>
					<td>
					<input type="text" name="komentar" path="komentar"/>
					</td>
				</tr>
  </tbody>
</table>
